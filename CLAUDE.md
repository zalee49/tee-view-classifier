...
When changing `N_FRAMES`/`SAMPLE_PERIOD` in `src/data.py`, update `MIN_FRAMES` in `stage_avis.py` and `_MIN_FRAMES` in `Pruning/prune.py` to match. (`prune.py` rejects clips whose `NumberOfFrames` header is below the threshold as an early pre-filter; `stage_avis.py` keeps the decode-based check as the safety net for MJPG truncation.)
